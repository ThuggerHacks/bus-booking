...
switch ($_GET['type']) {
    ...
    case 'username':
        ...
        if ($res->num_rows == 0)
            echo '<span class="text-success">Usuario disponivel</span>';
        else
            echo '<span class="text-danger">Usuario indisponivel</span>';
        $conn->close();
        break;
    ...
    case 'email':
        if (!filter_var($_GET['q'], FILTER_VALIDATE_EMAIL))
            die('<span class="text-danger">Email invalido</span>');
        $conn = initDB();
        $res = $conn->query("select id from users where email='" . $_GET['q'] . "'");
        if ($res->num_rows == 0)
            echo '';
        else
            echo '<span class="text-danger">Email ja existe</span>';
        $conn->close();
        break;
    ...
    case 'showseats':
        ...
        $con->close();
        echo '<div class="modal modal-lg" tabindex="-1" role="dialog" id="seats" style="display: block">
        ...
        <div class="modal-content">
            ...
            if (!isset($_GET['ticket'])) {
                ...
                <input type="submit" class="btn btn-primary" value="Comprar" name="buy"/>
                </form>';
            }
            echo '<a onclick="$(\'#seatViewer\').hide()"><button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button></a>
            </div>
            <script async>
